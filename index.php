	...
	protected function contentParentHtml($menu_id, $parent_id) {
		...
		if( count($results) > 0 ){
			...
			foreach ($results as $result) {
				...
				?> 
				<?php 
				$href = $this->url->link('design/menu/groupDelete', 'token=' . $this->session->data['token'] . '&menu_id=' . $this->request->get['menu_id'] . '&menu_group_id=' . $result['menu_group_id'], 'SSL');
				?>
				
				<?php 
				$outrun .='<div class="dd-delete">
					<a href="'.$href.'" class="confirmation"  class="confirmation">
					sss
					</a>
				</div>';
				?>
				<a href="<?php echo $href; ?>" class="confirmation">Link</a>
...
		...
		$this->load->view('design/menu_list');
		return $outrun;
	}
